...
set.seed(2)
bst.cv = xgb.cv(param=param,  xgbd ,nrounds=500 ,early_stopping_rounds=50,nfold = 10,seed=2,maximize =T)#,"maximize"=F,niter=10,  ,niter=500, nrounds = 50,colsample_bytree =0.9,gamma =100,min_child_weight=10,subsample =0.9)#)#prediction=TRUE, verbose=FALSE,
bst.cv$evaluation_log

plot(bst.cv$evaluation_log[,iter],bst.cv$evaluation_log[,test_auc_mean])



set.seed(2)
bst <- xgboost(param=param,xgbd,nrounds=42 ,nfold = 10,seed=2)#,niter=299, early_stopping_rounds=50,min_child_weight=10,subsample =0.9)#
...
set.seed(2)
btr$pred=predict(bst,xgbd,ntreelimit = 42)#
hist(btr$pred)
...
set.seed(2)
btr$pred=predict(bst,xgbdtest,ntreelimit = 42)#,ntreelimit = 40
hist(btr$pred)
btr$pred=as.numeric(btr$pred>0.5)
a=table(btr$went_on_backorder,btr$pred)
a
...
pre= predict(bst,xgbdtest_t,ntreelimit = 42)
pre=as.numeric(pre>0.15)
table(pre)
